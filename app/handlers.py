...
import os
from states import UserState, user_data, init_user_data
...
def get_subscription_keyboard():
    ...
    builder.add(InlineKeyboardButton(
        ...
        url=f"https://t.me/{env.CHANNEL_USERNAME}")
    )
    builder.add(InlineKeyboardButton(
...
@router.message(F.media_group_id, F.content_type.in_({'photo', 'document'}))
async def handle_albums(message: types.Message, album: list[types.Message], bot):
    user_id = message.from_user.id
...
async def handle_files(message: types.Message, bot):
...
@router.callback_query(lambda c: c.data == "to_text")
async def to_text_callback(callback: types.CallbackQuery):
    user_id = callback.from_user.id

    if user_id not in user_data:
        await callback.message.answer("Пожалуйста, начните с команды /start", show_alert=True)
        return

    if len(user_data[user_id]["photos"]) == 0:
        await callback.message.answer("Сначала пришлите хотя бы одно фото", show_alert=True)
        return
...
@router.message()
async def text_handler(message: types.Message, bot):
    user_id = message.from_user.id
